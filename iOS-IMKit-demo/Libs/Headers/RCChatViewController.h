...
@interface RCChatViewController
    : RCBasicViewController <RCSendMessageDelegate,
                             RCLocationPickerViewControllerDelegate> {
...
@property(nonatomic, strong) RCChatSessionInputBarView *msgInputBar;

@property(nonatomic, strong) RCMultiActionView *multiActionView;
@property(nonatomic, strong) RCEmojiView *emojiView;

@property(nonatomic, strong) RCConversationTableHeaderView *tableHeaderView;
 ...
 */
- (void)sendLBSMessage:(UIImage *)mapScreenShot
 ...
 *  发送会话消息
 *
 *  @param messageContent 消息体，支持文字，图片，语音，地理位置，图文消息
 */
- (void)sendChatMessage:(RCMessageContent *)messageContent;
